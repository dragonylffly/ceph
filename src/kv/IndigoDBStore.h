...
#ifndef INDIGO_DB_STORE_H
#define INDIGO_DB_STORE_H
...
#include "os/bluestore/BitmapFreelistManager.h"
...
enum
{
  l_indigodb_first = 34300,
  l_indigodb_gets,
  l_indigodb_txns,
  l_indigodb_txns_sync,
  l_indigodb_get_latency,
  l_indigodb_submit_latency,
  l_indigodb_submit_sync_latency,
  l_indigodb_compact,
  l_indigodb_compact_range,
  l_indigodb_compact_queue_merge,
  l_indigodb_compact_queue_len,
  l_indigodb_write_wal_time,
  l_indigodb_write_memtable_time,
  l_indigodb_write_delay_time,
  l_indigodb_write_pre_and_post_process_time,
  l_indigodb_last,
};
...
const int64_t SUPER_RESERVED = (int64_t)(8192);
const int64_t DISK_SIZE = (int64_t)1 << (int64_t)31;
const int64_t ALLOCATE_UNIT = (int64_t)(1 << 12);

const string PREFIX_INDIGO = "V";
const string PREFIX_INDIGO_META = "P";
const string PREFIX_INDIGO_BITMAP = "p";

/**
 * Uses RocksDB to implement the KeyValueDB interface
 */
class IndigoDBStore : public KeyValueDB
{
  CephContext *cct;
  PerfCounters *logger;
  string path;
  ...
  public:
    explicit CompactThread(IndigoDBStore *d) : db(d) {}
    void *entry() override
    {
      db->compact_thread_entry();
      return NULL;
    }
    friend class IndigoDBStore;
  } compact_thread;
...
public:
  ...
  IndigoDBStore(CephContext *c, const string &path, void *p) : cct(c),
                                                               logger(NULL),
                                                               path(path),
                                                               priv(p),
                                                               db(NULL),
                                                               ...
                                                               fm(NULL),
                                                               alloc(NULL),
                                                               compact_queue_lock("IndigoDBStore::compact_thread_lock"),
  ...
  struct RocksWBHandler : public rocksdb::WriteBatch::Handler
  {
             ...
             const rocksdb::Slice &value) override
    {
      ...
      seen += "\nPut( Prefix = " + prefix + " key = " + pretty_binary_string(key_to_decode) + " Value size = " + std::to_string(size) + ")";
      num_seen++;
    }
    void SingleDelete(const rocksdb::Slice &key) override
    {
      string prefix((key.ToString()).substr(0, 1));
      string key_to_decode((key.ToString()).substr(2, string::npos));
      seen += "\nSingleDelete(Prefix = " + prefix + " Key = " + pretty_binary_string(key_to_decode) + ")";
      num_seen++;
    }
    void Delete(const rocksdb::Slice &key) override
    {
      string prefix((key.ToString()).substr(0, 1));
      string key_to_decode((key.ToString()).substr(2, string::npos));
      seen += "\nDelete( Prefix = " + prefix + " key = " + pretty_binary_string(key_to_decode) + ")";
               ...
               const rocksdb::Slice &value) override
    {
      ...
      string key_to_decode((key.ToString()).substr(2, string::npos));
      uint64_t size = (value.ToString()).size();
      seen += "\nMerge( Prefix = " + prefix + " key = " + pretty_binary_string(key_to_decode) + " Value size = " + std::to_string(size) + ")";
  ...
  public:
    ...
    explicit RocksDBTransactionImpl(IndigoDBStore *_db);
    void set(
        const string &prefix,
        const string &k,
  ...
  int get(
      ...
      std::map<string, bufferlist> *out) override;
  int get(
      const string &prefix,
      const string &key,
  ...
  public:
    explicit RocksDBWholeSpaceIteratorImpl(rocksdb::Iterator *iter) : dbiter(iter) {}
