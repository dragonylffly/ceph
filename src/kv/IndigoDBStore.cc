...
//
class IndigoDBStore::MergeOperatorRouter : public rocksdb::AssociativeMergeOperator
{
  IndigoDBStore &store;
...
public:
  ...
  MergeOperatorRouter(IndigoDBStore &_store) : store(_store) {}
...
int IndigoDBStore::set_merge_operator(
    const string &prefix,
...
rocksdb::Logger *create_indigodb_ceph_logger()
{
...
int IndigoDBStore::tryInterpret(const string &key, const string &val, rocksdb::Options &opt)
{
...
int IndigoDBStore::ParseOptionsFromString(const string &opt_str, rocksdb::Options &opt)
{
...
int IndigoDBStore::open_bdev()
{
  assert(bdev == NULL);
  string p = "/dev/vdc";
  bdev = BlockDevice::create(cct, p, NULL, static_cast<void *>(this));
  int r = bdev->open(p);
  if (r < 0)
    goto fail;
  /*
...
int IndigoDBStore::create_fm()
{
  /*
  {
    KeyValueDB::Transaction t = get_transaction();
    t->rmkeys_by_prefix(PREFIX_INDIGO);
    t->rmkeys_by_prefix(PREFIX_INDIGO_META);
    t->rmkeys_by_prefix(PREFIX_INDIGO_BITMAP);
    submit_transaction_sync(t);
  }*/
  //FreelistManager *fm = FreelistManager::create(g_ceph_context, "bitmap", this, PREFIX_INDIGO_ALLOC);
  FreelistManager *fm = new BitmapFreelistManager(g_ceph_context, this, PREFIX_INDIGO_META, PREFIX_INDIGO_BITMAP);
  bool create = true;
  {
    bufferlist bl;
    if (get(PREFIX_INDIGO, "create", &bl) == 0)
      create = false;
  }
  if (create == false)
  {
    if (fm->init(DISK_SIZE) < 0)
    {
      dout(0) << "init fm error" << dendl;
      delete fm;
      return -1;
    }
    dout(0) << "fm init success" << dendl;
    this->fm = fm;
    return 0;
  }
  KeyValueDB::Transaction t = get_transaction();
  {
    bufferlist bl;
    bl.append("bitmap");
    t->set(PREFIX_INDIGO, "freelist_type", bl);
  }
  ...
  fm->allocate(0, reserved, t);
  {
    bufferlist bl;
    bl.append("success");
    t->set(PREFIX_INDIGO, "create", bl);
  }
  if (submit_transaction_sync(t))
  {
    dout(0) << "fm create error" << dendl;
    delete fm;
    return -1;
  }

  {
    ...
    bufferlist bl;
    int ret = get(PREFIX_INDIGO_BITMAP, k, &bl);
    dout(0) << "ret: " << ret << " length: " << bl.length() << dendl;
    //dout(0) << " 0x" << std::hex << first_key << std::dec << ": ";
  ...
  dout(0) << "fm create success" << dendl;
  this->fm = fm;
  return 0;
}

int IndigoDBStore::init_allocator()
{
  //Allocator *alloc = Allocator::create(g_ceph_context, "bitmap", DISK_SIZE, ALLOCATE_UNIT);
  ...
  dout(0) << "allocator init success" << dendl;
  this->alloc = alloc;
  return 0;
}

int IndigoDBStore::init(string _options_str)
{
...
int IndigoDBStore::create_and_open(ostream &out)
{
  if (env)
  {
...
int IndigoDBStore::do_open(ostream &out, bool create_if_missing)
{
  rocksdb::Options opt;
  ...
  PerfCountersBuilder plb(g_ceph_context, "rocksdb", l_indigodb_first, l_indigodb_last);
  plb.add_u64_counter(l_indigodb_gets, "get", "Gets");
  plb.add_u64_counter(l_indigodb_txns, "submit_transaction", "Submit transactions");
  plb.add_u64_counter(l_indigodb_txns_sync, "submit_transaction_sync", "Submit transactions sync");
  plb.add_time_avg(l_indigodb_get_latency, "get_latency", "Get latency");
  plb.add_time_avg(l_indigodb_submit_latency, "submit_latency", "Submit Latency");
  plb.add_time_avg(l_indigodb_submit_sync_latency, "submit_sync_latency", "Submit Sync Latency");
  plb.add_u64_counter(l_indigodb_compact, "compact", "Compactions");
  plb.add_u64_counter(l_indigodb_compact_range, "compact_range", "Compactions by range");
  plb.add_u64_counter(l_indigodb_compact_queue_merge, "compact_queue_merge", "Mergings of ranges in compaction queue");
  plb.add_u64(l_indigodb_compact_queue_len, "compact_queue_len", "Length of compaction queue");
  plb.add_time_avg(l_indigodb_write_wal_time, "rocksdb_write_wal_time", "Rocksdb write wal time");
  plb.add_time_avg(l_indigodb_write_memtable_time, "rocksdb_write_memtable_time", "Rocksdb write memtable time");
  plb.add_time_avg(l_indigodb_write_delay_time, "rocksdb_write_delay_time", "Rocksdb write delay time");
  plb.add_time_avg(l_indigodb_write_pre_and_post_process_time,
  ...
  {
    bufferlist bl;
    if (get(PREFIX_INDIGO, "magics", &bl) == 0)
    {
      dout(0) << "magics:" << bl << dendl;
    }
    else
    {
      //KeyValueDB::Transaction t = get_transaction();
      ...
      extents.reserve(9);
      dout(0) << "size:" << alloc->get_free() << dendl;
      alloc->allocate(ALLOCATE_UNIT, ALLOCATE_UNIT, ALLOCATE_UNIT, 0, &extents);
      for (auto &p : extents)
      {
        dout(0) << "offset: " << p.offset << ", length: " << p.length << dendl;
      }
    }
  }
  /*
...
int IndigoDBStore::_test_init(const string &dir)
{
...
void IndigoDBStore::split_stats(const std::string &s, char delim, std::vector<std::string> &elems)
{
  std::stringstream ss;
  ss.str(s);
  std::string item;
...
void IndigoDBStore::get_statistics(Formatter *f)
{
  if (!g_conf->rocksdb_perf)
  {
  ...
  if (g_conf->rocksdb_collect_extended_stats)
  {
    ...
    f->open_object_section("indigodbstore_perf_counters");
...
int IndigoDBStore::submit_transaction(KeyValueDB::Transaction t)
{
  ...
  *_dout << " Rocksdb transaction: " << bat_txc.seen << dendl;
  ...
  if (g_conf->rocksdb_perf)
  {
    ...
    logger->tinc(l_indigodb_write_memtable_time, write_memtable_time);
    logger->tinc(l_indigodb_write_delay_time, write_delay_time);
    logger->tinc(l_indigodb_write_wal_time, write_wal_time);
    logger->tinc(l_indigodb_write_pre_and_post_process_time, write_pre_and_post_process_time);
  }

  logger->inc(l_indigodb_txns);
  logger->tinc(l_indigodb_submit_latency, lat);

  return s.ok() ? 0 : -1;
}

int IndigoDBStore::submit_transaction_sync(KeyValueDB::Transaction t)
{
  utime_t start = ceph_clock_now();
  // enable rocksdb breakdown
  // considering performance overhead, default is disabled
  ...
  *_dout << " Rocksdb transaction: " << bat_txc.seen << dendl;
  ...
  logger->inc(l_indigodb_txns_sync);
  logger->tinc(l_indigodb_submit_sync_latency, lat);

  return s.ok() ? 0 : -1;
}

IndigoDBStore::RocksDBTransactionImpl::RocksDBTransactionImpl(IndigoDBStore *_db)
{
  db = _db;
}

void IndigoDBStore::RocksDBTransactionImpl::set(
    const string &prefix,
    const string &k,
...
void IndigoDBStore::RocksDBTransactionImpl::rmkey(const string &prefix,
                                                  const string &k)
{
...
void IndigoDBStore::RocksDBTransactionImpl::rm_single_key(const string &prefix,
                                                          const string &k)
{
  bat.SingleDelete(combine_strings(prefix, k));
}

void IndigoDBStore::RocksDBTransactionImpl::rmkeys_by_prefix(const string &prefix)
{
...
void IndigoDBStore::RocksDBTransactionImpl::rm_range_keys(const string &prefix,
                                                          const string &start,
                                                          const string &end)
{
  if (db->enable_rmrange)
...
void IndigoDBStore::RocksDBTransactionImpl::merge(
    const string &prefix,
    const string &k,
    const bufferlist &to_set_bl)
{
    ...
    std::map<string, bufferlist> *out)
{
  ...
  utime_t lat = ceph_clock_now() - start;
  logger->inc(l_indigodb_gets);
  logger->tinc(l_indigodb_get_latency, lat);
  return 0;
}
...
int IndigoDBStore::split_key(rocksdb::Slice in, string *prefix, string *key)
{
  size_t prefix_len = 0;
...
void IndigoDBStore::compact()
{
  logger->inc(l_indigodb_compact);
  rocksdb::CompactRangeOptions options;
  db->CompactRange(options, nullptr, nullptr);
}

void IndigoDBStore::compact_thread_entry()
{
  compact_queue_lock.Lock();
  while (!compact_queue_stop)
  {
    while (!compact_queue.empty())
    {
      ...
      logger->set(l_indigodb_compact_queue_len, compact_queue.size());
      compact_queue_lock.Unlock();
      logger->inc(l_indigodb_compact_range);
...
void IndigoDBStore::compact_range_async(const string &start, const string &end)
{
  ...
  while (p != compact_queue.end())
  {
    ...
    if (p->first <= end && p->first > start)
    {
      ...
      logger->inc(l_indigodb_compact_queue_merge);
      break;
    }
...
bool IndigoDBStore::check_omap_dir(string &omap_dir)
{
  rocksdb::Options options;
  options.create_if_missing = true;
  rocksdb::DB *db;
...
void IndigoDBStore::compact_range(const string &start, const string &end)
{
  rocksdb::CompactRangeOptions options;
...
int64_t IndigoDBStore::request_cache_bytes(PriorityCache::Priority pri, uint64_t chunk_bytes) const
{
...
int64_t IndigoDBStore::get_cache_usage() const
{
  return static_cast<int64_t>(bbt_opts.block_cache->GetUsage());
}

int64_t IndigoDBStore::commit_cache_size()
{
...
int64_t IndigoDBStore::get_cache_capacity()
{
  return bbt_opts.block_cache->GetCapacity();
}

IndigoDBStore::RocksDBWholeSpaceIteratorImpl::~RocksDBWholeSpaceIteratorImpl()
{
  delete dbiter;
}
int IndigoDBStore::RocksDBWholeSpaceIteratorImpl::seek_to_first()
{
  dbiter->SeekToFirst();
  assert(!dbiter->status().IsIOError());
  return dbiter->status().ok() ? 0 : -1;
}
int IndigoDBStore::RocksDBWholeSpaceIteratorImpl::seek_to_first(const string &prefix)
{
...
int IndigoDBStore::RocksDBWholeSpaceIteratorImpl::seek_to_last()
{
  dbiter->SeekToLast();
  assert(!dbiter->status().IsIOError());
  return dbiter->status().ok() ? 0 : -1;
}
int IndigoDBStore::RocksDBWholeSpaceIteratorImpl::seek_to_last(const string &prefix)
{
...
int IndigoDBStore::RocksDBWholeSpaceIteratorImpl::upper_bound(const string &prefix, const string &after)
{
...
int IndigoDBStore::RocksDBWholeSpaceIteratorImpl::lower_bound(const string &prefix, const string &to)
{
...
bool IndigoDBStore::RocksDBWholeSpaceIteratorImpl::valid()
{
  return dbiter->Valid();
}
int IndigoDBStore::RocksDBWholeSpaceIteratorImpl::next()
{
  if (valid())
  {
    dbiter->Next();
...
int IndigoDBStore::RocksDBWholeSpaceIteratorImpl::prev()
{
  if (valid())
  {
    dbiter->Prev();
...
string IndigoDBStore::RocksDBWholeSpaceIteratorImpl::key()
{
  string out_key;
  split_key(dbiter->key(), 0, &out_key);
  return out_key;
}
pair<string, string> IndigoDBStore::RocksDBWholeSpaceIteratorImpl::raw_key()
{
  string prefix, key;
...
bool IndigoDBStore::RocksDBWholeSpaceIteratorImpl::raw_key_is_prefixed(const string &prefix)
{
...
bufferlist IndigoDBStore::RocksDBWholeSpaceIteratorImpl::value()
{
  return to_bufferlist(dbiter->value());
}

size_t IndigoDBStore::RocksDBWholeSpaceIteratorImpl::key_size()
{
  return dbiter->key().size();
}

size_t IndigoDBStore::RocksDBWholeSpaceIteratorImpl::value_size()
{
  return dbiter->value().size();
}

bufferptr IndigoDBStore::RocksDBWholeSpaceIteratorImpl::value_as_ptr()
{
  rocksdb::Slice val = dbiter->value();
  return bufferptr(val.data(), val.size());
}

int IndigoDBStore::RocksDBWholeSpaceIteratorImpl::status()
{
  return dbiter->status().ok() ? 0 : -1;
}

string IndigoDBStore::past_prefix(const string &prefix)
{
  string limit = prefix;
  limit.push_back(1);
  return limit;
...
IndigoDBStore::WholeSpaceIterator IndigoDBStore::_get_iterator()
{
