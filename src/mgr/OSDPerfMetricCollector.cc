...
bool is_limited(const std::map<OSDPerfMetricQueryID,
                                boost::optional<OSDPerfMetricLimit>> &limits) {
  for (auto &it : limits) {
    if (!it.second) {
      return false;
    }
...
void OSDPerfMetricCollector::process_reports(
  ...
  for (auto &it : reports) {
    ...
    for (auto &it : report.group_packed_performance_counters) {
      auto &key = it.first;
      ceph::buffer::list::iterator bl_it = (const_cast<ceph::buffer::list&>(it.second)).begin();
      ...
      for (size_t i = 0; i < query.performance_counter_descriptors.size(); i++) {
        ...
        //const_cast<PerformanceCounterDescriptor&>(*desc_it).unpack_counter(bl_it, &c);
        const_cast<PerformanceCounterDescriptors::value_type&>(*desc_it).unpack_counter(bl_it, &c);
