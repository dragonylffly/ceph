    ...
    bool create)
{
  ...
  //std::string tem;
  //get_object_key(store->cct, oid, &tem);
  //ldout(store->cct, 0) << "key: " << url_escape(tem) << dendl;
  Onode *on;
  if (v.length() == 0)
  {
    assert(r == -ENOENT);
...
int BlueStore::_open_db(bool create)
{
  ...
  if (do_bluefs)
  {
    ...
    if (kv_backend != "rocksdb" && kv_backend != "indigodb")
    {
  ...
  BitmapFreelistManager::setup_merge_operator(db, "p");
