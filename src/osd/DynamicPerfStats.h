...
public:
  ...
  void add(const OSDService *osd, const pg_info_t &pg_info, OpRequest& op,
    ...
    for (auto &it : data) {
      ...
      for (auto &limit : limits.at(query)) {
        ...
        for (; it_counters != counters.end(); it_counters++) {
          wsum += it_counters->second[index].first;
          if (util::generate_random_number(0, wsum) <=
              it_counters->second[index].first) {
            auto i = util::generate_random_number(0, limit.max_count - 1);
